...
- **Retrair/expandir itens de lançamentos múltiplos**: na tabela de Lançamentos, o selo "N itens"
  de um lançamento múltiplo agora é um botão — clique para expandir e ver as sub-linhas de cada
  item, ou para retrair de volta e deixar a tabela mais compacta. Por padrão os itens começam
  retraídos.
  - Novos botões **Expandir todos** e **Retrair todos** no cabeçalho da tabela de Lançamentos,
    para abrir ou fechar de uma vez os itens de todos os lançamentos múltiplos visíveis.
  - Acessível: o botão usa `aria-expanded` e o leitor de tela anuncia quando os itens são
    expandidos ou retraídos.
- **Paginação padronizada em todas as tabelas**: além da tabela de Lançamentos (que já tinha),
  agora também têm paginação com as mesmas opções — **10, 25, 50, 100, 200 ou Todos** — as
  tabelas de:
  - **Documentos** guardados na nuvem.
  - **Correções Aprendidas (IA)**.
  - **Gerenciar Listas** (uma paginação independente por campo: Banco/Cartão, Categoria, Credor,
    Devedor, Forma de Pagamento, Estabelecimento).
  - **Contas e Cartões** (aba Saldos).
  - A preferência de itens por página é salva por tabela no navegador, e a navegação (Primeira/
    Anterior/Próxima/Última) segue o mesmo padrão visual já usado em Lançamentos.
